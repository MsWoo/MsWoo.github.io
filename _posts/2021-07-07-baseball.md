...
1. 첫번째 if문 `if tri[i] in gameAnswer` 입력받은 숫자의 1번째 원소가 정답에 포함되는지 체크합니다.  (위치 상관 없이 값이 있는지 확인)  
...
1. `if(scount == 4)` 입력한 숫자가 모두 값과 위치를 맞았다면  
...
1. 정답이 아닌 경우 ball과 strike를 출력하고, 다시 while loop을 돕니다.

```{.python}
print("%d S %d B" %(scount, bcount))
```

1. 정답을 맞출 때까지 loop로 값을 입력받고 정답을 체크해야하므로,  
...
1. 꼭 필요하진 않지만 정답을 알고싶을때 999를 입력하면 정답을 출력하고  
